...
public class Mole : MonoBehaviour
{
    ...
    [Header("HitSounds")]
    [SerializeField] AudioSource moleHitSound;
    [SerializeField] AudioSource moleHatBroke;
    [SerializeField] AudioSource bombExplode;
    ...
    {
        // make sure we start at the start position
        ...
        while (elapsed < showDuration)
        {
            ...
            // changes the box collider so it isnt hittable
        ...
        while (elapsed < showDuration)
        {
            ...
            // change the box collider to grow with the mole
    ...
    private void OnMouseDown()
    {
        if (hittable)
        {
            ...
            clickParticles.transform.position = new Vector2(mousePos.x, mousePos.y);
            ParticleSystem.MainModule psmain = clickParticles.main;
            switch (moleType)
            {
                case MoleType.Standard:
                    {
                        spriteRenderer.sprite = moleHit;
                        // changes and triggers the particle system
                        psmain.startColor = new Color(0.3867925f, 0.3867925f, 0.3867925f);
                        clickParticles.Play();
                        // plays sound effect
                        moleHitSound.Play();
                        // add score
                        ...
                        // so you cant keep hitting the mole for points
                        hittable = false;
                        break;
                    }
                case MoleType.HardHat:
                    {
                        if (lives == 2)
                        {
                            ...
                            psmain.startColor = new Color(1f, 0.8f, 0.007843138f);
                            clickParticles.Play();
                            moleHatBroke.Play();
                            lives--;
                        }
                ...
                case MoleType.Bomb:
                    {
                        psmain.startColor = new Color(0.996f, 0.4666667f, 0.003921569f);
                        clickParticles.Play();
                        bombExplode.Play();
    ...
    private void SetLevel(int level)
    {
        ...
        stayDuration = Random.Range(durationMin, durationMax);
    }
    private void Awake()
    {
